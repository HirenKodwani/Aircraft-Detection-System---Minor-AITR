...
# Backend integration
engine = CameraDetectionEngine(enable_database=True, enable_api=True)
print(f"✅ Backend ready - Session: {engine.session_id}")
...
def handle_connect():
    ...
    emit('status', {'message': 'Connected', 'session_id': engine.session_id})


@socketio.on('sensor_data')
def handle_sensor_data(data):
    """Receive GPS and compass data from browser"""
    # Just accept it, don't need to do anything special
    pass
...
def handle_get_nearby_aircraft(data):
    ...
    try:
        ...
        radius = data.get('radius', 50)  # nautical miles
        ...
        if aircraft_data and 'ac' in aircraft_data:
            ...
            print(f"✈️ Found {len(aircraft_list)} aircraft within {radius}nm")
        else:
...
def handle_process_frame(data):
    ...
    try:
        ...
        # Run YOLO - Same as Camera module
        ...
        if len(results) > 0:
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    print(f"🎯 {cls_name.upper()} | {conf:.0%} | {enhanced['threat_level'].upper()}")
